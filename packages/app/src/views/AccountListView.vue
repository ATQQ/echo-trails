...
<template>
  <div class="account-list-container">
    <van-nav-bar title="账号管理" left-text="返回" left-arrow @click-left="router.back()" placeholder class="safe-padding-top"
      right-text="新增" @click-right="showAddUser = true; formData = { username: '', operator: '', token: '' }" />

    <div class="content">
      <van-collapse v-model="activeNames" accordion>
        <van-collapse-item v-for="user in userList" :key="user.username"
          :title="user.username + (user.isSystem ? ' (系统)' : '')" :name="user.username">
          <van-cell-group>
            <van-cell v-for="op in user.operators" :key="op.name" :title="op.name">
              <template #right-icon>
                <van-button size="small" type="primary" @click.stop="openUpdatePassword(user, op.name)"
                  v-if="!user.isSystem">修改密码</van-button>
    ...
    <!-- Dialogs -->
    <van-dialog v-model:show="showAddUser" title="新增账号组" show-cancel-button
      :before-close="(action) => action === 'confirm' ? handleAddUser() : true">
      <van-cell-group inset>
        <van-field v-model="formData.username" label="家庭" placeholder="请输入账户名" />
        <van-field v-model="formData.operator" label="用户名" placeholder="请输入操作员名" />
    ...
    <van-dialog v-model:show="showAddOperator" title="新增成员" show-cancel-button
      :before-close="(action) => action === 'confirm' ? handleAddOperator() : true">
    ...
    <van-dialog v-model:show="showUpdatePassword" title="修改密码" show-cancel-button
      :before-close="(action) => action === 'confirm' ? handleUpdatePassword() : true">
      <div class="dialog-content">
        <van-cell-group inset>
          <van-cell title="家庭" :value="formData.username" />
          <van-cell title="用户名" :value="formData.operator" />
